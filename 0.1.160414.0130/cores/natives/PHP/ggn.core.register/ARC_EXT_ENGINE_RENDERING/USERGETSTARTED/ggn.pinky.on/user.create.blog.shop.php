	...
	*/
		...
		$Bloc = (new Theme\tag(['class'=>' enable col-16 gui flex column']));
			...
			if(is_array($_AccountType) && isset($_AccountType['child'][$_THIS_BLOG_TYPE]) && is_array($_AccountType['child'][$_THIS_BLOG_TYPE])){
				...
				$tpl->head->title = 'Créez une nouvelle boutique';

				$tpl->head->script('ggn.pinky/user.create.blog.js');
					...
					*/
						$Form = (new Theme\Tag([
							...
							$Form->tag->Bloc->tag->Fields = (new Theme\Tag([
								...
								/* Nom */
								->text('<div class="field-input xl styled gui box-rounded gui flex row center box-shadow-dark" id="create-blog-blog-title-field"><span class="gui iconx icon" title="Minimum : 3  Maximun : 32">'.$_BlogType['iconx'].'</span><input type="text" name="blog-title" placeholder="Nom de la boutique" ggn-handler-focus="Gabarit.Input.Focus" gabarit-focus="#create-blog-blog-title-field,#create-blog-blog-title-info" focus-class="focus,enable" required pattern=".{3,32}"></div>')
								...
								$STYLIVOIR->getCriterionsToDPO($Form->tag->Bloc->tag->Fields, 'boutiques');
