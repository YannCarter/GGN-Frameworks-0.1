		...
		$Bloc = (new Theme\tag(['class'=>' enable col-16 gui flex column']));
			...
			if(!is_array($_AccountType) || !isset($_AccountType['child'][$_THIS_BLOG_TYPE])){
				...
				$Bloc->tag->Content->tag->Info = (new Theme\Tag([
					...
					->text('<div class="xh3 align-center padding-t-x20"><span class="gui icon close color-primary"></span></div>')
			...
			if(is_array($_AccountType) && isset($_AccountType['child'][$_THIS_BLOG_TYPE]) && is_array($_AccountType['child'][$_THIS_BLOG_TYPE])){
				...
				$tpl->head->title = 'Créez un nouveau blog casting';
					...
					*/
						$Form = (new Theme\Tag([
							...
							$Form->tag->Bloc->tag->Fields = (new Theme\Tag([
								...
								/* Nom */
								->text('<div class="field-input xl styled gui box-rounded gui flex row center box-shadow-dark" id="create-blog-blog-title-field"><span class="gui iconx icon" title="Minimum : 3  Maximun : 32">'.$_BlogType['icon'].'</span><input type="text" name="blog-title" placeholder="Nom du blog casting" ggn-handler-focus="Gabarit.Input.Focus" gabarit-focus="#create-blog-blog-title-field,#create-blog-blog-title-info" focus-class="focus,enable" required pattern=".{3,32}"></div>')
								...
								/* Description */
								->text('<div class="field-input xl styled gui box-rounded gui flex row center box-shadow-dark" id="create-blog-about-field"><span class="gui iconx icon">mode_edit</span><textarea name="about" placeholder="Description du salon d\'esthétique" ggn-handler-focus="Gabarit.Input.Focus" gabarit-focus="#create-blog-about-field,#create-blog-about-info"  focus-class="focus,enable" ></textarea></div>') 
								...
								->text('<div class="gui box info text-x12 padding-x12 box-rounded disable" id="create-blog-city-info">La localisation de votre boutique</div>')
								...
								->text('<div class="label align-left padding-x16 text-x18">Plus de détails </div>')
								...
								$STYLIVOIR->getCriterionsToDPO($Form->tag->Bloc->tag->Fields, 'castings', true, true);
							...
							$Form->tag->Bloc->tag->Fields
								...
								/* Autre */
								->text('<div class="field-input xl styled gui box-rounded gui flex row center box-shadow-dark" id="create-blog-other-detail-field"><span class="gui iconx icon" >add</span><input type="text" name="keywords" placeholder="Décrivez-vous plus encore" ggn-handler-focus="Gabarit.Input.Focus" gabarit-focus="#create-blog-other-detail-field,#create-blog-other-detail-info" focus-class="focus,enable" ></div>')
